...
public class OfferPredictionEngine {
	...
	/**
	 * this is the main method exposed to the rest API for the user. it analyzes
	 * the user's buying pattern and generates the list of offers suggestion
	...
	public List<OfferDTO> showOfferPredictions() throws UserNotFoundException {
		...
		List<TransactionHistory> transactionHistoriesForUser = getTransactionHistoryForUser(
				staticData.getTransactionHistories(), user.getId());
		if (transactionHistoriesForUser.isEmpty()) {
			...
			filtersForSuggestions = findFiltersFromTransactionsHistory(filtersForSuggestions,
					transactionHistoriesForUser);
		}
		OfferStoresHelper offerStoreHelper = new OfferStoresHelper();

		List<OfferDTO> offerSuggestion = offerStoreHelper.getOfferSuggestion(filtersForSuggestions, this.staticData,
				location);
	...
	/**
	 * find the filter criteria from user buying habit,which is then used to
	 * make offer recommendation in his locality.
	 * 
	 * @param filterCriteriaList
	 * @param transactionHistories
	 * @return
	 */
	private List<FilterCriteria> findFiltersFromTransactionsHistory(List<FilterCriteria> filterCriteriaList,
		...
		for (TransactionHistory transactionHistory : transactionHistories) {
			...
			/*
			 * transactions older than two months wont be used to make
			 * suggestions
			 */
			...
			/*
			 * setting the filter criteria 1. category name 2. price segement
			 */
